...
# add these lines to your LaTeX document
...
# checks if any of the given operators are not enclosed by parentheses
free.operators <- function(x, operators){
  v <- strsplit(x, split = "")[[1]]
  if(length(intersect(v, operators)) == 0){
    return(FALSE)
  }
  else{
    a <- 0
...
# if the string includes a "tall" pattern, LaTeX tall parentheses are used
enclose <- function(x){
  tall <- c("\\(", "sin", "cos", "tan", "log", "ln", "frac", "sqrt")
  tall.pattern <- paste0("(", tall, ")", collapse = "|")
  
  if(grepl(pattern = tall.pattern, x = x)){
...
# checks if the first and last character of a string are
# matching parentheses
is.enclosed <- function(x){
  v <- strsplit(x, split = "")[[1]]
  if(v[1] == "(" & v[length(v)] == ")"){
    a <- 0
    for(v0 in v[-length(v)]){
      if(v0 == "("){
        a <- a + 1
      }
      else if(v0 == ")"){
